...
const fetchCart = async () => {
    const newItemId = params.get('id')
    const newItemName = params.get('title')
    const newItemPrice = params.get('price')
    ...
    if (newItemId && newItemName && newItemPrice) {
        ...
        message.innerHTML = `Success! Added <strong>${newItemName}</strong> to the shopping cart`
        return json
    } else {
        message.innerHTML = `Failure! Unable to add a movie`
    }
}
...
window.onload = async event => {
    ...
    displayCart(json)
    document.getElementById('continue').href = "movieList.html?" + localStorage.getItem("searchParameter") + '&single_page=1'
